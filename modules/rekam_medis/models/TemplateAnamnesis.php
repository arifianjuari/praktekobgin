...
class TemplateAnamnesis
{
    ...
    public function __construct($conn = null)
    {
        try {
            ...
            error_log("Database Error in TemplateAnamnesis constructor: " . $e->getMessage());
            throw new PDOException("Gagal menginisialisasi koneksi database: " . $e->getMessage());
        } catch (Exception $e) {
            error_log("Error in TemplateAnamnesis constructor: " . $e->getMessage());
            throw new Exception("Gagal menginisialisasi model: " . $e->getMessage());
        }
    }

    /**
     * Mendapatkan semua kategori template anamnesis
     * 
     * @return array
     */
    public function getAllKategori()
    {
        try {
            ...
            $kategori = [
                ['kategori_anamnesis' => 'fetomaternal'],
                ['kategori_anamnesis' => 'ginekologi umum'],
                ['kategori_anamnesis' => 'onkogin'],
                ['kategori_anamnesis' => 'fertilitas'],
                ['kategori_anamnesis' => 'uroginekologi']
            ];
    ...
    /**
     * Mendapatkan template anamnesis berdasarkan kategori
     * 
    ...
    {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM template_anamnesis WHERE kategori_anamnesis = ? AND status = 'active' ORDER BY nama_template_anamnesis ASC");
    ...
    /**
     * Mendapatkan semua template anamnesis
     * 
     * @return array
     */
    ...
    {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM template_anamnesis WHERE status = 'active' ORDER BY kategori_anamnesis ASC, nama_template_anamnesis ASC");
            $stmt->execute();
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
    ...
    /**
     * Mendapatkan template anamnesis berdasarkan ID
     * 
    ...
    {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM template_anamnesis WHERE id_template_anamnesis = ? AND status = 'active'");
    ...
    /**
     * Mencari template anamnesis berdasarkan keyword
     * 
    ...
    public function searchTemplate($keyword)
    {
        try {
            ...
            $stmt = $this->pdo->prepare("SELECT * FROM template_anamnesis 
                WHERE (nama_template_anamnesis LIKE ? OR isi_template_anamnesis LIKE ? OR kategori_anamnesis LIKE ? OR tags LIKE ?) 
                AND status = 'active' 
                ORDER BY kategori_anamnesis ASC, nama_template_anamnesis ASC");
    ...
    public function saveTemplate($data)
    {
        try {
            // Validasi data
            if (empty($data['nama_template_anamnesis']) || empty($data['isi_template_anamnesis']) || empty($data['kategori_anamnesis'])) {
                throw new PDOException("Data template tidak lengkap");
            }

            // Generate ID template baru
            $stmt = $this->pdo->prepare("SELECT MAX(CAST(SUBSTRING(id_template_anamnesis, 3) AS UNSIGNED)) as max_id FROM template_anamnesis");
            $stmt->execute();
            ...
            $id_template = 'AN' . str_pad($next_id, 6, '0', STR_PAD_LEFT);

            // Insert data
            $stmt = $this->pdo->prepare("INSERT INTO template_anamnesis 
                (id_template_anamnesis, nama_template_anamnesis, isi_template_anamnesis, kategori_anamnesis, status, tags) 
                VALUES (?, ?, ?, ?, ?, ?)");

            $result = $stmt->execute([
                $id_template,
                $data['nama_template_anamnesis'],
                $data['isi_template_anamnesis'],
                $data['kategori_anamnesis'],
                $data['status'],
                $data['tags']
            ]);
    ...
    public function updateTemplate($data)
    {
        try {
            // Validasi data
            if (empty($data['id_template_anamnesis']) || empty($data['nama_template_anamnesis']) || 
                empty($data['isi_template_anamnesis']) || empty($data['kategori_anamnesis'])) {
                throw new PDOException("Data template tidak lengkap");
            }

            // Update data
            $stmt = $this->pdo->prepare("UPDATE template_anamnesis 
                SET nama_template_anamnesis = ?, 
                    isi_template_anamnesis = ?, 
                    kategori_anamnesis = ?, 
                    status = ?,
                    tags = ?,
                    updated_at = CURRENT_TIMESTAMP
                WHERE id_template_anamnesis = ?");
    ...
    /**
     * Menghapus template anamnesis (hard delete)
     * 
     * @param string $id_template
     * @return bool
     */
    public function deleteTemplate($id_template)
    {
        try {
            ...
            // Delete data
            $stmt = $this->pdo->prepare("DELETE FROM template_anamnesis WHERE id_template_anamnesis = ?");
