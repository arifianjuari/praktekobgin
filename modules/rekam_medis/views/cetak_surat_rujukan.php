...
<head>
  ...
  <title>Surat Rujukan - <?= htmlspecialchars($surat['nomor_surat'] ?? '-') ?></title>
  <style>
...
<div class="container">
  ...
  <div class="section">
    <div>Dengan ini merujuk pasien berikut untuk pemeriksaan/penanganan lebih lanjut:</div>
    <div class="section">
      ...
      <div><span class="label">Keperluan</span><span class="value">: <?= htmlspecialchars($surat['keperluan'] ?? '-') ?></span></div>
      <div><span class="label">Tanggal Surat</span><span class="value">: <?= htmlspecialchars($surat['tanggal_surat']) ?></span></div>
    </div>
    <div>Dimohon kesediaannya untuk menindaklanjuti sesuai keahlian dan fasilitas yang tersedia.</div>
  </div>
