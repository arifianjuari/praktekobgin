        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title">Manajemen Template Anamnesis</h3>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <form action="index.php" method="get" id="formFilter">
                                ...
                                <input type="hidden" name="action" value="template_anamnesis">
                                <div class="input-group">
                                    <select id="filterKategori" name="kategori" class="form-select">
                                        <option value="">Semua Kategori</option>
                                        <?php foreach ($kategori as $k) : ?>
                                            <option value="<?= $k['kategori_anamnesis'] ?>" <?= isset($filter_kategori) && $filter_kategori == $k['kategori_anamnesis'] ? 'selected' : '' ?>><?= ucwords($k['kategori_anamnesis']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <form action="index.php" method="get" id="formSearch">
                                ...
                                <div class="input-group">
                                    ...
                                    <?php if (isset($search_keyword) || isset($filter_kategori)) : ?>
                                        <a href="index.php?module=rekam_medis&action=template_anamnesis" class="btn btn-outline-secondary">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <?php foreach ($templates as $template) : ?>
                                    <tr>
                                        <td><?= $template['id_template_anamnesis'] ?></td>
                                        <td><?= $template['nama_template_anamnesis'] ?></td>
                                        <td>
                                            <div style="max-height: 100px; overflow-y: auto;">
                                                <?= nl2br(htmlspecialchars($template['isi_template_anamnesis'])) ?>
                                            </div>
                                        </td>
                                        <td><?= ucwords($template['kategori_anamnesis']) ?></td>
                                        <td>
                                        ...
                                        <td>
                                            <form method="post" action="index.php?module=rekam_medis&action=edit_template_anamnesis_form" style="display:inline;">
                                                <input type="hidden" name="id_template" value="<?= $template['id_template_anamnesis'] ?>">
                                                <button type="submit" class="btn btn-sm btn-info mb-1 w-100">
                                                    <i class="fas fa-edit"></i> Edit
                                                </button>
                                            </form>
                                            <button type="button" class="btn btn-sm btn-danger w-100" onclick="confirmDelete('<?= $template['id_template_anamnesis'] ?>')">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTambahTemplateLabel">Tambah Template Anamnesis</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="index.php?module=rekam_medis&action=simpan_template_anamnesis" method="post" id="formTambahTemplate">
                    <div class="mb-3">
                        <label for="nama_template_anamnesis" class="form-label">Nama Template <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="nama_template_anamnesis" name="nama_template_anamnesis" required>
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="kategori_anamnesis" name="kategori_anamnesis" required>
                            ...
                            <?php foreach ($kategori as $k) : ?>
                                <option value="<?= $k['kategori_anamnesis'] ?>"><?= ucwords($k['kategori_anamnesis']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="isi_template_anamnesis" class="form-label">Isi Template <span class="text-danger">*</span></label>
                        <textarea class="form-control" id="isi_template_anamnesis" name="isi_template_anamnesis" rows="10" required></textarea>
                    </div>
                    <div class="mb-3">
...
<form id="formHapusTemplate" action="index.php?module=rekam_medis&action=hapus_template_anamnesis" method="post" style="display: none;">
