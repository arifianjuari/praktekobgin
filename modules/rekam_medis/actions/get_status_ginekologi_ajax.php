...
// Log untuk debugging
error_log("AJAX Request received for status ginekologi no_rkm_medis: $no_rkm_medis");

try {
    // Prepared statement untuk menghindari SQL injection
    $sql = "SELECT * FROM status_ginekologi WHERE no_rkm_medis = ? ORDER BY created_at DESC";
    ...
    // Log untuk debugging
    error_log("Status ginekologi data found: " . count($data) . " rows for no_rkm_medis: $no_rkm_medis");

    // Kirim respons sukses
    echo json_encode([
        'status' => 'success',
        'message' => 'Data status ginekologi berhasil diambil',
        'data' => $data
    ]);
} catch (Exception $e) {
